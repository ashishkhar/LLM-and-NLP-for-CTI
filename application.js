...
require(["domReady!", "stix2viz/stix2viz/stix2viz"], function (document, stix2viz) {
     ...
     *
     * @param edgeDataSet A visjs DamatplotlibtaSet instance with graph edge data derived
     *      from STIX content
    ...
    function renderDataFromJson(data) {
        try {
            let customConfig = {}; // You can add custom configuration if needed

            // Hard-coded working icon directory setting for this application.
            customConfig.iconDir = "stix2viz/stix2viz/icons";

            let [nodeDataSet, edgeDataSet, stixIdToObject] = stix2viz.makeGraphData(data, customConfig);

            // Example: Always show as graph view
            view = stix2viz.makeGraphView(
    ...
    // Fetching data.json and rendering it
    fetch('data.json')
        .then(response => response.json())
        .then(data => {
            renderDataFromJson(data);
        })
        .catch(error => {
